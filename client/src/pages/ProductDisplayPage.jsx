...
import image2 from '../assets/imagePrice.png'
...
const ProductDisplayPage = () => {
  ...
  const handleScrollRight = () => {
    ...
    if (imageContainer.current.scrollLeft + imageContainer.current.clientWidth >= imageContainer.current.scrollWidth) {
      return; // Prevent scrolling if already at the end
    }
  ...
  const handleScrollLeft = () => {
    ...
    if (imageContainer.current.scrollLeft <= 0) {
      return; // Prevent scrolling if already at the beginning
    }
    imageContainer.current.scrollLeft -= 100
  }
    ...
    <section className='w-full min-h-screen p-4'>
      <section className='container mx-auto p-4 grid lg:grid-cols-2 '>
        ...
        <div className='p-4 lg:pl-7 text-base lg:text-lg'>
          ...
          <h2 className='text-lg font-semibold lg:text-3xl font-[cursive]'>{data.name}</h2>
          <p className='text-blue-600'>{data.unit}</p>
          <Divider />
          <div>
            <p className=''>Price</p>
            <div className='flex items-center gap-2 lg:gap-4'>
              <div className='border border-amber-600 px-4 py-2 rounded bg-amber-50 w-fit'>
                <p className='font-semibold text-lg lg:text-xl'>{DisplayPriceInRupees(pricewithDiscount(data.price, data.discount))}</p>
              </div>
              {
                data.discount && (
                  <p className='line-through text-red-300'>{DisplayPriceInRupees(data.price)}</p>
                )
              }
              {
                data.discount && (
          ...
          <h2 className='font-semibold'>HStore provide this service? </h2>
          <div>
      ...
      {/* <section className="w-full rounded-lg  p-4 my-8 mx-auto max-w-3xl">
        ...
        <div className="flex mt-4">
          ...
          <div className="ml-3">
            <div className="font-medium text-purple-800">HStore</div>
            <div className="text-gray-600">Posted on 2025-10-08 14:30</div>
            <div className="mt-2 ">This is a sample comment. Lorem ipsum dolor sit amet, consectetur
              adipiscing elit.
            </div>
          </div>
        </div>
      ...
      {/* <div className="flex mt-4">
          ...
          <div className="ml-3">
            ...
            <div className="mt-2 text-purple-800">Another sample comment. Sed quis velit auctor, bibendum dolor in,
              accumsan tellus.
            </div>
          </div>
        </div> */}
